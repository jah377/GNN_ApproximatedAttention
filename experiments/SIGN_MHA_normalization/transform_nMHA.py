...
from model_nDPA import DotProductAttention
...
def DPAttention(data, K: int, attn_heads: int = 1):
    ...
    adj_t = extract_attention(data, attn_heads, norm=None)
    adj_t = deg_inv_sqrt.view(-1, 1) * adj_t * deg_inv_sqrt.view(1, -1)
...
def extract_attention(data, attn_heads, norm=None):
    ...
    return model(data.x, data.edge_index, norm)
