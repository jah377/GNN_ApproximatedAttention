...
def eval(data, model, loader, evaluator):
    ...
    for batch in loader:
        xs = [data.x[batch].to(device)]
        xs += [data[f'x{hop}'][batch].to(device)
               for hop in range(1, model.hop + 1)]
        logits, batch_time = inference(model, xs)

        loss = F.nll_loss(logits, data.y[batch].to(device))
...
def main(config):
    ...
    # import data
    file_name = f'{config.DATASET}_sign_k{config.HOPS}.pth'
    ...
    prev_loss, trigger_times = 1e10, 0
