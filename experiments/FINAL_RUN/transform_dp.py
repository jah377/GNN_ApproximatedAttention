...
def sparse_min_max_norm(s_coo):
    ...
    v = s_csr.data
    nnz = s_csr.getnnz(axis=1)
    idx = np.r_[0, nnz[:-1].cumsum()]

    max_r = np.maximum.reduceat(v, idx)
    min_r = np.minimum.reduceat(v, idx)
    min_r *= (nnz == shape[1])  # if not fully-connected, min=0

    # create matrices for normalization
...
def dotproduct_filter(data, args):
    ...
    assert hasattr(args, 'DPA_NORM'), 'Must specify DPA_NORM value'
